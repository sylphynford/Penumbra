...
/datum/species/proc/kicked(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	if(!get_dist(user, target))
		if(!stander && (user.mobility_flags & MOBILITY_STAND))
			...
			var/selzone = accuracy_check(user.zone_selected, user, target, /datum/skill/combat/unarmed, user.used_intent)
			var/obj/item/bodypart/affecting = target.get_bodypart(check_zone(selzone))
			var/damage = user.get_punch_dmg() + 1
...
/datum/species/proc/apply_damage(damage, damagetype = BRUTE, def_zone = null, blocked, mob/living/carbon/human/H, forced = FALSE, spread_damage = FALSE)
	...
	switch(damagetype)
		if(BRUTE)
			var/final_damage = base_damage * brutemod * H.physiology.brute_mod
			H.damageoverlaytemp = 20
			if(!HAS_TRAIT(H, TRAIT_NOPAIN))
				if(final_damage > 5)
					H.AdjustSleeping(-50)
					if(prob(final_damage * 3))
						if(final_damage > (10 + (H.STACON > 10 ? (H.STACON-10) * 5 : (H.STACON-10) * 1)))
							H.emote("painscream")
						else
							H.emote("pain")
				if(final_damage > (10 + (H.STACON > 10 ? (H.STACON-10) * 5 : (H.STACON-10) * 1)) && !HAS_TRAIT(H, TRAIT_NOPAINSTUN))
				//	H.Immobilize(8)
					shake_camera(H, 2, 2)
					H.stuttering += 5
				if(final_damage > 10 && !HAS_TRAIT(H, TRAIT_NOPAINSTUN))
					H.Slowdown(clamp(final_damage/10, 1, 5))
					shake_camera(H, 1, 1)
				if(final_damage < 10)
			...
			if(BP)
				if(BP.receive_damage(final_damage, 0))
					H.update_damage_overlays()
			else
				H.adjustBruteLoss(final_damage)
		if(BURN)
			var/final_damage = base_damage * burnmod * H.physiology.burn_mod
			H.damageoverlaytemp = 20
			if(final_damage > 10 && prob(final_damage))
				H.emote("pain")
			if(final_damage < 10)
				H.flash_fullscreen("redflash1")
			else if(final_damage < 20)
				...
				H.flash_fullscreen("redflash3")
			if(BP)
				if(BP.receive_damage(0, final_damage))
					H.update_damage_overlays()
			else
