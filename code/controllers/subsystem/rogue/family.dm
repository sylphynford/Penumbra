...
/datum/family/proc/addRel(var/mob/living/carbon/human/target, var/mob/living/carbon/human/holder,var/rel_type, var/announce = FALSE) //creates a relation for two members.
	...
	if(rel_type == REL_TYPE_SPOUSE && length(SSfamily.family_candidates))
		var/datum/job/holder_job = SSjob.GetJob(holder.job)
		var/datum/job/target_job = SSjob.GetJob(target.job)
		
		// Only give rings to non-baron/consort spouses who aren't already family members
		if(!(istype(holder_job, /datum/job/roguetown/lord) || istype(holder_job, /datum/job/roguetown/lady)) && \
			!(istype(target_job, /datum/job/roguetown/lord) || istype(target_job, /datum/job/roguetown/lady)) && \
			members[holder.name] == null)
			...
			var/obj/item/clothing/ring/soul/target_ring = new(target, holder.real_name)
			target.equip_to_slot_if_possible(target_ring, SLOT_RING)
